...
#define MAXVALUELEN (10 * MaxFileName)
  ...
  while (*pos) {
	...
	case 9:  strcpy(dayofweek,value);
	  break;
	case 10:  wakeup = atoi(value); shutdown = wakeup; // set shutdown backward compatible
	  break;
  ...
  while(strstr(name, "!^pipe^!"))
     strreplace(name, "!^pipe^!", "|");
  while(strstr(cmd, "!^pipe^!"))
     strreplace(cmd, "!^pipe^!", "|");
...
{
    char* tmp_name = replaceSpecialChars(name);
    char* tmp_cmd = replaceSpecialChars(cmd);

    char* buffer = NULL;
    if (asprintf(&buffer, "%ld:%s:%d:%s:%s:%s:%s:%s:%s:%d:%d:%d:%d:%d", 
	     id,
	     tmp_name, 
	     enabled,
	     tmp_cmd,
