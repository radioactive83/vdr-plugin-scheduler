...
class cLogFile: public cFile
{
 public:
    ...
    void Open(const char* filename, const char* version)
	{
		...
		esyslog("TASKER: could not open log file: %s", filename);
	    Log(1, "---------------------------------------", loglevellimit);
	    Log(1, "Tasker log started (verbose level %d, version %s)", loglevellimit, version);
	}
    void Log(int LogLevel, const char *text, ...)
	{
		...
		va_list Arg; 
		va_start(Arg,text); 
		if (vasprintf(&buffer, text, Arg) == -1)
		  return;
		va_end(Arg);
		...
		*p++ = ' ';
		...
		char* log = NULL;
		if (asprintf(&log, "%s %s: %s\n", datebuf, timebuf, buffer) == -1) return;
		free(buffer);
    ...
    void eSysLog(const char *text, ...)
	{
	    ...
	    va_list Arg; 
	    va_start(Arg,text); 
	    if (vasprintf(&buffer, text, Arg) == -1) return; 
	    va_end(Arg);
	    esyslog("Tasker: %s", buffer);
	    Log(1, "%s", buffer);
	    free(buffer);
	}
